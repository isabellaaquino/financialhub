    ...
    <Transition appear show={props.isOpen} as={Fragment}>
      ...
      >
        <Dialog
          ...
          className="absolute mt-6 top-0 left-1/2 -translate-x-1/2 flex justify-center cursor-pointer z-999"
          onClose={(e) => null}
        >
          <Dialog.Panel
            className={`rounded-2xl ${alertProps.color} p-6 shadow-xl transition-all`}
          >
            <div className="flex flex-row gap-5">
              <div>
                <span className="select-none material-symbols-rounded">
                  {alertProps.icon}
                </span>
              </div>
              <div className="select-none font-medium text-xl">
                {props.message}
              </div>
            </div>
          </Dialog.Panel>
